...
- ~~**Final step — prettier gate + repo-wide format sweep.**~~ Closed: ran `npm run format` (45 files; the bulk was `src/styles/global.css` at ~3.3K lines, as predicted) and added `npx prettier --check .` to CI after lint, before tests. Landed as two commits — the mechanical reformat (`git show -w` clean) and the CI gate — on top of the `game/` removal so the sweep didn't touch the legacy build. `game` was already dropped from `.prettierignore` in the removal PR.
