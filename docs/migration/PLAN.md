...
- **Investigate whether `#root` and its peer surfaces actually need their own background.** Related to the prior fix that added `#root, .big-demo, .kb` to the theme-transition list (`game/index.html` commit `25030ce`, mirrored at `src/styles/global.css:2217`): the content area's `background: var(--cream)` (`src/styles/global.css:1454`) was originally added to "prevent dark/light background bleed-through during screen fade transitions" — but with `body` already painting `var(--cream)` underneath, `#root` could just be `transparent` and inherit. If that holds, drop both the `background` rule and the entire `#root, .big-demo, .kb` block from the theme-transition list — there's nothing to transition. Probably the same logic applies to `.card` / `.info-card` / `.dead-end-banner` *if* they only ever sit on the cream surface; if any of them appears against a contrasting background, they keep their fill. Worth surveying every selector in the theme-transition rule and deleting the ones that exist only because something below them was opaque when it shouldn't have been. Verify the cross-fade between screens (the `.stagger` fadeIn animation) doesn't bleed through to the body's pre-transition color when the elements are transparent — that's the bleed-through the original comment guarded against.
