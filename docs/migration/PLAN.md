...
- **Set `VITE_APPS_SCRIPT_URL` in the production env** (Vercel project settings) before flipping DNS — EmailSignup throws on empty string at form-submit time, not at build time, so the build will succeed even if the env var is missing. After promoting the build, submit the form once against the live URL and confirm the row lands in the Sheet — the build is not the end-to-end check. **Deploy blocker; manual signup smoke test required.**
...
- **Address `setTimeout` cleanup gaps in `PlayScreen`.** Surfaced during Phase 4e review: `triggerShake` and the three `onEnd`-via-timeout calls (lines 149 / 208 / 228) have no cleanup. If the user navigates home / restarts within the timeout window, the deferred callback fires after unmount — `setShaking(false)` warns in dev, the deferred `onEnd` is technically safe (App is mounted) but the pattern is fragile. The `useReducer` refactor (next item) naturally absorbs the timer logic into reducer effects; either land the cleanup as part of the reducer PR or as a precursor. Becomes a hard test failure once Phase 5's RTL sweep adds an `App` smoke that exercises mode transitions (RTL's `act()` surfaces the warnings).
- **PlayScreen `useReducer` refactor** (Phase 1b deep-review carryover). PlayScreen has 11 `useState` + 11 mirror `useRef`s — the mirror-state pattern exists because event handlers needed fresh values via stale closures. With a reducer, dispatch is stable and reducer reads the latest state; most refs become unnecessary. Now that Phase 4 has all the types in place, this refactor has compile-time scaffolding. Recommend landing **before** Phase 5's RTL test sweep, since the test surface gets simpler once state is reducer-managed.
- Update `README.md` with new dev/build commands and the `VITE_APPS_SCRIPT_URL` env var requirement.
- **RTL coverage gaps from Phase 5d**: `StartScreen.test.tsx` and `OnboardingScreen.test.tsx` were not added in the 5d sweep (heavy children require module mocks; deferred to keep 5d focused). Both are user-facing flows worth pinning before deploy. `App.test.tsx` for the screen-transition state machine (start → playing → end → start) is also worth one test — App-level wiring is currently uncovered. Land these as a 5d-followup or fold into 5e.
